...
# Setup logging early so it's available everywhere
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Try to import C++ module - may not be available in all environments
try:
    import quant_risk_engine

    CPP_ENGINE_AVAILABLE = True
except ImportError as e:
    logger.warning(f"C++ quant_risk_engine module not available: {e}")
    logger.warning(
        "Some features may be limited. Build C++ module with cmake if needed."
    )
    quant_risk_engine = None
...
def validate_portfolio_item(item: Dict[str, Any], index: int) -> None:
    ...
    for field in required_fields:
        if field not in item:
            raise ValueError(
                f"Portfolio item {index}: missing required field '{field}'"
            )

    option_style = item.get("style", "european").lower()
    if option_style not in ["european", "american"]:
        raise ValueError(
            f"Portfolio item {index}: style must be 'european' or 'american'"
        )
    ...
    if pricing_model not in ["blackscholes", "binomial", "jumpdiffusion"]:
        raise ValueError(
            f"Portfolio item {index}: pricing_model must be 'blackscholes', 'binomial', or 'jumpdiffusion'"
        )
...
def validate_market_data(asset_id: str, md: Dict[str, Any]) -> None:
    ...
    for field in required_fields:
        if field not in md:
            raise ValueError(
                f"Market data for '{asset_id}': missing required field '{field}'"
            )

    if not isinstance(md["spot"], (int, float)) or md["spot"] <= 0:
        raise ValueError(
            f"Market data for '{asset_id}': spot must be a positive number"
        )
    ...
    if not isinstance(md["vol"], (int, float)) or md["vol"] < 0:
        raise ValueError(
            f"Market data for '{asset_id}': vol must be a non-negative number"
        )

    if "dividend" in md:
        if not isinstance(md["dividend"], (int, float)) or md["dividend"] < 0:
            raise ValueError(
                f"Market data for '{asset_id}': dividend must be a non-negative number"
            )
...
def auto_fetch_missing_market_data(
    portfolio_assets: set, provided_market_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    if missing_assets:
        raise ValueError(
            f"Could not fetch market data for: {', '.join(missing_assets)}. Please provide manually or check ticker symbols."
        )
...
def create_option(item: Dict[str, Any]) -> Any:
    if not CPP_ENGINE_AVAILABLE:
        raise RuntimeError(
            "C++ quant_risk_engine module is not available. Build with cmake first."
        )
...
# Reconfigure app with static folder for dashboard
app.static_folder = str(DASHBOARD_DIR)
app.static_url_path = ""
...
def update_market_data():
    ...
    try:
        ...
        app.logger.error(
            f"Unexpected error in update_market_data: {traceback.format_exc()}"
        )
    ...
    try:
        ...
        try:
            complete_market_data = auto_fetch_missing_market_data(
                portfolio_assets, market_data_map_py
            )
...
def price_option():
    ...
    try:
        ...
        if not market_data_provided or not all(
            k in market_data_provided for k in ["spot", "rate", "vol"]
        ):
            fetcher = get_market_data_fetcher()
            try:
                # Try cache first
                ...
                if cached_data:
                    md_data = {
                        ...
                        "dividend": market_data_provided.get(
                            "dividend", cached_data.get("dividend", 0.0)
                        ),
                    }
                    auto_fetched = True
                ...
                return jsonify(
                    {
                        "error": f"Could not fetch market data for {asset_id}: {str(e)}. Please provide manually."
                    }
                ), 400
        else:
        ...
        return jsonify(
            {
                ...
                "direction": "long"
                if net_quantity > 0
                else ("short" if net_quantity < 0 else "flat"),
            }
        ), 200
    ...
    try:
        ...
        instrument_counts = {
            "european": sum(
                1
                for item in portfolio_data
                if item.get("style", "european") == "european"
            ),
            "american": sum(
                1
                for item in portfolio_data
                if item.get("style", "european") == "american"
            ),
            "calls": sum(
                1 for item in portfolio_data if item["type"].lower() == "call"
            ),
